...
function BookingGrup () {
    ...
    const searchRoom = (e) => {
        ...
        .catch((err) => {
            ...
            if(value.startDate == null || value.endDate == null){
                setTitle('Please input check in and check out date')
                errorModal.showModal()
            }

            if(selectedCustomer == ''){
                setTitle('Please select customer grup representative')
                errorModal.showModal()
            }
    ...
    const getCustomerGrup = (e) => {
        e?.preventDefault()
        const loading = document.getElementById('loading_modal')
        loading.showModal()

        axios.get(`/customer/grup`, {
            headers: {
    ...
    const confirm = () => {
        ...
        axios.post('/reservasi/grup/store', data,{
            headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`
            }
        }
    ...
    return(
        <>
                    ...
                    <div className="flex items-center mt-5 mx-5">
                        <div className="bg-base-100 card mt-5 shadow-2xl w-full flex justify-between items-center flex-col px-5 py-2">
                            <div className='p-3 bg-base-100 rounded-xl flex flex-col items-center justify-center gap-4 lg:w-8/12  w-full'>
                                ...
                                <div className="form-control w-full">
                                    <select className="select select-bordered" value={selectedCustomer} onChange={(e)=>setSelectedCustomer(e.target.value)}>
                                        <option disabled selected value=''>Select Customer Grup Representative</option>
                                        {
                                            customer.map((item, index) => {
                                                return (
                                                    <option key={index} value={item.id}>{item.nama+' - '+item.nama_institusi}</option>
                                                )
                                            })
                                        }
                                    </select>
            ...
            <LoadingModal />
            <Error id="error_input_date" title={title} message="" button={"i am sorry"} onClick={()=>document.getElementById("error_input_date").close()}/>
            ...
            If you have any special requests or need to modify or cancel your reservation, please contact us at support@gah.com." button={"Wonderfull"} onClick={()=>(document.getElementById("success").close(), navigate("/reservation"))}/>        </>
    )
}

export default BookingGrup;